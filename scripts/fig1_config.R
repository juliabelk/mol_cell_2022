...
sub_clusters <- "Subset_10_400_LSI"

valid_bc <- read.delim(paste0("barcode_info/fig1_cellColData_trim.csv"),sep=",")[[1]]
sub_sel <- function(x) {
...
get_constraints <- function(proj,scrna) {
  groupList <- SimpleList(
    IL4 = SimpleList(
      ATAC = proj$cellNames[which(proj$Sample == "IL4")],
      RNA = rownames(scrna@meta.data)[which(scrna@meta.data$orig.ident == "IL4")]
    ),  
    IFNG = SimpleList(
